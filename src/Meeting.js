    ...
    return (
      <div>
        {lines.map((line, index) => {
          ...
          return (
            <p key={index} style={{ color: highlight ? '#EEEFB7' : 'white' }}>
              {line}
            </p>
          );
        })}
            ...
            <React.Fragment>
              <div>
                ...
                <div className="chatbot-message">
                  <ContentComponent content={message.text} />
                </div>
              </div>
            </React.Fragment>
          )}
...
function InputText(props) {    // 입력창
  ...
  const handleSendMessage = async (e) => {
    ...
    const meetingMinutes = props.filesContent.map((file, index) => {
      const fileName = props.files[index].name;
      return `<${index + 1}번째 회의록/ 파일명: ${fileName}>\n(${file[0].content})\n`;
    }).join("");
    ...
    const messages = [        // 요약본에서 물어보는 프롬프트
      ...
      {
        "role": "system",
        "content": "여기에는 사용자가 제공한 질문에 대한 정보를 주어진 회의록을 바탕으로 최대한 정확하고 간단하게 개괄식으로만 필요한 정보를 응답합니다. 답변 시 시작하는 문장에 '건설사 간의 갈등은 다음과 같습니다:'와 같은 서론 없이 바로 개조식으로 응답하십시오. 답변의 마지막에는 다음 예시의 형태로 참조한 회의록의 파일명을 명시하시오.(예시:\n\n▶ 응답 참조 파일\n팀내 회의 회의록.pdf\nA건설 곽두팔.pdf "
      }
    ];
    ...
    try {
      ...
      const newMessage = {
        id: Date.now(),
        sender: "bot",
        text: "앗, 사용 가능한 리소스가 일시적으로 부족합니다. 잠시 후에 다시 시도해 주세요.",
      };
...
function FlowChat() { // flowchat button
  ...
  const [isWord, setIsWord] = useState('단어');
...
function Meeting() {
  ...
  return (
    <div className="Meeting">
      ...
      <InputText files={files} filesContent={filesContent} onButtonClick={handleChange} onSendMessage={handleSendMessage} onUploadFile={handleUpload} />
    </div>
  );
}
